"""
Enhanced Sokoban Game Logger

This module provides a comprehensive, colorful logging setup using loguru.
Features beautiful console output and structured file logging for the Sokoban game.
"""
...
def log_level_generation(level_size: tuple, boxes: int, success: bool = True, **metrics):
    """Log level generation events."""
    gen_logger = get_logger("LevelGenerator")
    
    if success:
        gen_logger.info(
            f"🗺️  Level generated successfully: {level_size[0]}x{level_size[1]} with {boxes} boxes",
            **metrics
        )
    else:
        gen_logger.error(
            f"🚫 Level generation failed: {level_size[0]}x{level_size[1]} with {boxes} boxes",
            **metrics
        )
